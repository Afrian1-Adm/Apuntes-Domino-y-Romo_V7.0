(function () {
    ...
    async function desbloquearAudio() {
        ...
        let muteAnterior = false;
        let gananciaAnterior = null;
        ...
        try {
            ...
            // IMPORTANTE: este play existe únicamente para desbloquear
            // el audio del navegador después del primer gesto del usuario.
            // Nunca debe escucharse la chiva durante este proceso.
            volumenAnterior = audio.volume;
            muteAnterior = audio.muted;
            gananciaAnterior = audioGain?.gain?.value ?? null;

            audio.muted = true;
            audio.volume = 0;

            // Algunos navegadores envían el MediaElement directamente al
            // grafo WebAudio y pueden ignorar el volumen del elemento.
            // Silenciamos también el GainNode para evitar cualquier sonido.
            if (audioGain?.gain) {
            ...
            if (audio) {
                try {
                    ...
                    audio.muted = muteAnterior;
                } catch (_) {}
            }

            if (audioGain?.gain && gananciaAnterior !== null) {
                audioGain.gain.value = gananciaAnterior;
            }
